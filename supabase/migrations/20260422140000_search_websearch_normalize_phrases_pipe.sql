-- Boolean / websearch path: normalize query so it matches our single concat search_vector.
-- 1) Double-quoted "phrases" become plain words (websearch otherwise uses <-> adjacency, which
--    almost never matches when terms live in different profile fields — unquoted multi-word
--    plainto_tsquery worked, quoted looked "broken").
-- 2) Pipe | is documented as OR in UI hints but websearch_to_tsquery treats | as punctuation
--    and ANDs tokens; rewrite | to OR.
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          AND cand.search_vector @@ websearch_to_tsquery('english', v_search_web)
        )
      )
      AND (
        v_adv_name IS NULL
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1
          FROM unnest(coalesce(ce.tags, ARRAY[]::text[])) AS t(tag)
          JOIN unnest(p_filter_skills) AS s(skill)
            ON lower(tag) = lower(skill)
        )
      )
      AND (
